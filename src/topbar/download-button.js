...
const saveAsVideo = async ({ store, pixelRatio, fps, onProgress }) => {
  ...
  const req = await fetch(
    'https://api.polotno.dev/api/renders?KEY=FxrTlLd7ZEYLWTSWqNeP',
    {
      method: 'POST',
      headers: {
  ...
  while (true) {
    const jobReq = await fetch(
      `https://api.polotno.dev/api/renders/${job.id}?KEY=FxrTlLd7ZEYLWTSWqNeP`
    );
    const jobData = await jobReq.json();
    if (jobData.status === 'done') {
      downloadFile(jobData.output, 'Video.mp4');
      break;
...
export const DownloadButton = observer(({ store }) => {
  ...
  const getName = () => {
    ...
    return words.join(' ').replace(/\s/g, '-').toLowerCase() || 'Untitled Design';
  };
          ...
          {type === 'mp4' && (
            <>
              <div style={{ padding: '10px', maxWidth: '180px', opacity: 0.8 }}>
                ...
                <a href="mailto:admin@inktechprinting.services">
